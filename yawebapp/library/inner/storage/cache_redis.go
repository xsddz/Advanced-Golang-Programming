...
func NewRedis(conf RedisConf) ([]*redis.Client, error) {
	...
	for _, host := range conf.Hosts {
		options := redis.Options{
			...
			DB:       conf.DefaultDB,                           // redis数据库index
		...
		clis = append(clis, redisCli)
	}
	return clis, nil
}

func NewRedisSentine(conf RedisConf) ([]*redis.Client, error) {
	hosts := []string{}
	for _, item := range conf.Hosts {
		hosts = append(hosts, fmt.Sprintf("%v:%v", item.IP, item.Port))
	}
